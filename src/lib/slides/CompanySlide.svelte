...
<BaseSlide className="lg:flex-row text-[#f5f5f7]">
	...
	<div
		class="container mx-auto flex w-full flex-col items-center justify-center px-4 sm:px-6 lg:max-w-7xl lg:flex-row lg:px-8"
	>
		<!-- Company Info Section -->
		<div class="flex h-full w-full flex-col justify-center space-y-6 p-2 lg:w-1/2">
			<!-- Title & Subtitle -->
			<div class="space-y-4">
				<h1
			...
			<!-- Company Details -->
			<div class="grid w-1/2 grid-cols-1 gap-4 pt-2 sm:grid-cols-2 lg:gap-6">
				{#if company.founded}
							...
							{#each company.achievements as achievement}
								<div class="flex items-start space-x-3">
									<div
										...
										style="background-color: {accentColor}"
									></div>
									<span class="text-xs leading-relaxed text-[#a1a1a6] lg:text-sm"
										>{achievement}</span
									>
								</div>
							{/each}
